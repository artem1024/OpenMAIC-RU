    ...
    <div className="space-y-6">
          ...
          <Card key={outline.id} className="relative">
            <CardHeader className="pb-3">
              <div className="flex items-center gap-3">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="slide">{t('outlinesEditor.slide')}</SelectItem>
                    <SelectItem value="quiz">{t('outlinesEditor.quiz')}</SelectItem>
                  </SelectContent>
                </Select>
                <Button
                  variant="ghost"
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label>{t('outlinesEditor.sceneDescription')}</Label>
                <Textarea
              ...
              <div className="space-y-2">
                <Label>{t('outlinesEditor.keyPoints')}</Label>
                <Textarea
              ...
              {outline.type === 'quiz' && (
                <div className="p-3 bg-muted/50 rounded-lg space-y-3">
                  <Label className="text-sm font-medium">{t('outlinesEditor.quizConfig')}</Label>
                  <div className="grid grid-cols-3 gap-3">
                    <div className="space-y-1">
                      <Label className="text-xs">{t('outlinesEditor.questionCount')}</Label>
                      <Input
                        type="number"
                    ...
                    <div className="space-y-1">
                      <Label className="text-xs">{t('outlinesEditor.difficulty')}</Label>
                      <Select
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="easy">{t('outlinesEditor.difficultyEasy')}</SelectItem>
                          <SelectItem value="medium">{t('outlinesEditor.difficultyMedium')}</SelectItem>
                          <SelectItem value="hard">{t('outlinesEditor.difficultyHard')}</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    <div className="space-y-1">
                      <Label className="text-xs">{t('outlinesEditor.questionType')}</Label>
                      <Select
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="single">{t('outlinesEditor.typeSingle')}</SelectItem>
                          <SelectItem value="multiple">{t('outlinesEditor.typeMultiple')}</SelectItem>
                          <SelectItem value="text">{t('outlinesEditor.typeText')}</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                  </div>
      ...
      {outlines.length === 0 && (
        <Card className="p-8 text-center">
          <p className="text-muted-foreground mb-4">{t('outlinesEditor.noOutlines')}</p>
          <Button variant="outline" onClick={addOutline} disabled={isLoading}>
            <Plus className="size-4 mr-1" />
      ...
      <div className="flex justify-between pt-4">
        ...
        <Button onClick={onConfirm} disabled={isLoading || outlines.length === 0}>
          {isLoading ? t('outlinesEditor.generating') : t('outlinesEditor.confirmGenerate')}
        </Button>
      </div>
    </div>
  );
