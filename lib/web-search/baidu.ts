...
async function fetchWebSearch(
  ...
  try {
    const res = await safeWebSearchFetch(
      'baidu',
      buildQianfanUrl(BAIDU_WEB_SEARCH_PATH, baseUrl),
      {
      method: 'POST',
...
async function fetchBaike(query: string, apiKey: string): Promise<WebSearchSource[]> {
  try {
    const res = await safeWebSearchFetch('baidu', buildBaikeUrl(query), {
      method: 'GET',
      headers: baiduHeaders(apiKey),
    });
...
async function fetchScholar(
  ...
  try {
    const res = await safeWebSearchFetch('baidu', buildScholarUrl(query, baseUrl), {
      method: 'GET',
      headers: baiduHeaders(apiKey),
    });
