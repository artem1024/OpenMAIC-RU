...
- **Language in images**: If the image contains text, labels, or annotations, the prompt MUST explicitly specify that all text in the image should be in the course language. For Russian (`ru-RU`) courses say "all labels in Russian (Cyrillic)" — never leave the labels' language unspecified, otherwise the model defaults to English. For purely visual images without text, language does not matter
...
"mediaGenerations": [
  {
    "type": "image",
    "prompt": "A colorful diagram showing the water cycle with evaporation, condensation, and precipitation arrows, all labels in Russian (Cyrillic)",
    "elementId": "gen_img_1",
    "aspectRatio": "16:9"
  }
]
