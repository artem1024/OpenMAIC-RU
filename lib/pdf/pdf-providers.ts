...
export async function parsePDF(
  ...
  switch (config.providerId) {
    ...
    case 'mineru-cloud': {
      // RU-fork: feature flag — managed mode default off. Enable in .env via MINERU_PDF_ENABLED=true.
      const cloudEnabled = (process.env.MINERU_PDF_ENABLED || '').toLowerCase() === 'true';
      if (!cloudEnabled) {
        throw new Error(
          'MinerU Cloud provider is disabled. Set MINERU_PDF_ENABLED=true to enable it.',
        );
      }
