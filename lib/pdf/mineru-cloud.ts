...
// RU-fork inv: harden SSRF for any URL we hit, including dynamic ones returned
// by the MinerU response (presigned upload + ZIP download). In production,
// reject any URL that resolves to a private/loopback/link-local address —
// prevents a compromised MinerU response from pivoting to internal services.
async function assertSafeUrl(url: string, context: string): Promise<void> {
  if (process.env.NODE_ENV !== 'production') return;
  const err = await validateUrlForSSRF(url);
  if (err) {
    throw new Error(`MinerU Cloud ${context}: blocked unsafe URL — ${err}`);
  }
}
...
async function parseMinerUZip(zipUrl: string): Promise<ParsedPdfContent> {
  ...
  await assertSafeUrl(zipUrl, 'ZIP download');
...
export async function parseWithMinerUCloud(
  ...
  await assertSafeUrl(apiRoot, 'baseUrl');
  ...
  // RU-fork inv: validate presigned URL returned by MinerU before connecting.
  await assertSafeUrl(uploadUrls[0], 'presigned upload URL');
